...
interface KaribuGiniazCollegeProps {
  onBack: () => void;
}

const KaribuGiniazCollege: React.FC<KaribuGiniazCollegeProps> = ({ onBack }) => {
  const { theme } = useTheme();
  const [completedLessons, setCompletedLessons] = useState<boolean[]>(() => {
    const saved = localStorage.getItem('completed_karibu-giniaz-college');
    return saved ? JSON.parse(saved) : Array(12).fill(false);
  });
  const [selectedLessonIndex, setSelectedLessonIndex] = useState<number | null>(() => {
    const saved = localStorage.getItem('selectedLessonIndex_karibu-giniaz-college');
    return saved ? parseInt(saved, 10) : null;
  });
  const [lastActiveLessonIndex, setLastActiveLessonIndex] = useState<number | null>(() => {
    const saved = localStorage.getItem('lastActive_karibu-giniaz-college');
    return saved ? parseInt(saved, 10) : null;
  });
  ...
  useEffect(() => {
    localStorage.setItem('completed_karibu-giniaz-college', JSON.stringify(completedLessons));
  }, [completedLessons]);

  useEffect(() => {
    if (selectedLessonIndex !== null) {
      localStorage.setItem('selectedLessonIndex_karibu-giniaz-college', selectedLessonIndex.toString());
      localStorage.setItem('lastActive_karibu-giniaz-college', selectedLessonIndex.toString());
      setLastActiveLessonIndex(selectedLessonIndex);
    } else {
      localStorage.removeItem('selectedLessonIndex_karibu-giniaz-college');
    }
  }, [selectedLessonIndex]);
  ...
  const checkCache = async () => {
    ...
    for (const lesson of lessons) {
      const stored = await getLesson(`lesson_Karibu Giniaz College_${lesson.title}`);
      if (stored) {
  ...
  const lessons = [
    { title: '1. Historia ya Giniaz College', description: 'Kujifunza chimbuko na malengo ya chuo.' },
    { title: '2. Dira na Dhamira ya Chuo', description: 'Kuelewa mwelekeo na malengo makuu ya Giniaz.' },
    { title: '3. Programu za Masomo', description: 'Kujua kozi mbalimbali zinazotolewa.' },
    { title: '4. Maisha ya Chuo', description: 'Kuelewa mazingira ya kujifunzia na shughuli za ziada.' },
    { title: '5. Mawasiliano na Uongozi', description: 'Kujifunza jinsi ya kuwasiliana na uongozi wa chuo.' },
    { title: '6. Kanuni za Chuo', description: 'Kuelewa sheria na kanuni za nidhamu.' },
    { title: '7. Fursa za Ujuzi wa Vitendo', description: 'Kujifunza umuhimu wa ujuzi wa vitendo.' },
    { title: '8. Ushirikiano na Wadau', description: 'Kuelewa jinsi chuo kinavyoshirikiana na sekta mbalimbali.' },
    { title: '9. Ukuaji wa Kitaaluma', description: 'Mbinu za kukuza vipaji na umahiri.' },
    { title: '10. Utamaduni wa Giniaz', description: 'Kuelewa maadili na utamaduni wa chuo.' },
    { title: '11. Huduma za Wanafunzi', description: 'Kujua huduma mbalimbali zinazotolewa kwa wanafunzi.' },
    { title: '12. Hitimisho na Mtazamo wa Baadaye', description: 'Kuangalia mbele kwa mafanikio.' },
  ];
  ...
  const handleRefreshSingleLesson = async (e: React.MouseEvent, index: number, lessonTitle: string) => {
    ...
    if (window.confirm(`Je, una uhakika unataka kurefresh na kuandika upya Somo la ${index + 1}: "${lessonTitle}"?`)) {
      ...
      try {
        const cacheKey = `lesson_Karibu Giniaz College_${lessonTitle}`;
  ...
  const handleRefreshCourse = async () => {
    if (window.confirm("Je, una uhakika unataka kurefresh na kufuta masomo yote ya kozi hii ili yaandikwe upya?")) {
      ...
      try {
        await clearCourseCache("Karibu Giniaz College");
        setCachedLessons(new Set());
        setCompletedLessons(Array(12).fill(false));
        localStorage.removeItem('completed_karibu-giniaz-college');
        localStorage.removeItem('selectedLessonIndex_karibu-giniaz-college');
        localStorage.removeItem('lastActive_karibu-giniaz-college');
  ...
  const downloadAllLessons = async () => {
    ...
    try {
      for (const lesson of lessons) {
        await generateLessonContentStream("Karibu Giniaz College", lesson.title, () => {});
        completedCount++;
        setDownloadProgress(Math.round((completedCount / total) * 100));
      }
      await checkCache();
      setStatusType('success');
      setStatusMessage("Masomo yote ya Karibu Giniaz College yamepakiwa kwa offline!");
      setTimeout(() => setStatusMessage(null), 8000);
    } catch (err) {
  ...
  const handleSelectLesson = (index: number) => {
    ...
    localStorage.setItem('lastActive_karibu-giniaz-college', index.toString());
  };

  if (selectedLessonIndex !== null) {
    return (
      <LessonContent 
        courseTitle="Karibu Giniaz College"
        ...
        onBack={() => {
          ...
          if (selectedLessonIndex + 1 < lessons.length) {
            ...
            localStorage.setItem('lastActive_karibu-giniaz-college', (selectedLessonIndex + 1).toString());
          } else {
            setLastActiveLessonIndex(null);
            localStorage.removeItem('lastActive_karibu-giniaz-college');
          }
  ...
  return (
    <div className="w-full max-w-full overflow-hidden">
      ...
      {allLessonsCompleted && (
        <div className={`${successTheme.bg} border-2 border-dashed p-6 rounded-lg mb-8 text-center relative overflow-hidden`}>
          ...
          <h3 className="text-2xl font-bold mb-2 flex items-center justify-center">
            ...
            Hongera Sana! Umekamilisha Kozi Nzima ya Karibu Giniaz College!
          </h3>
          <p className={`${successTheme.muted} text-sm max-w-xl mx-auto`}>
            Umejifunza misingi yote ya chuo kuanzia dira, dhamira, programu na fursa za ujuzi. Umefanya uamuzi bora wa kuanza safari yako hapa!
          </p>
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
        <div>
          <h2 className={`text-3xl font-bold mb-2 ${currentTheme.titleText}`}>Karibu Giniaz College</h2>
          <p className={`${currentTheme.mutedText}`}>Chagua kipengele unachotaka kujifunza kuhusu Giniaz College.</p>
        </div>
        <button
...
export default KaribuGiniazCollege;
